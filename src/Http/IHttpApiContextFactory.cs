...
public interface IHttpApiContextFactory
{
    ...
    IApiContext<K, T> Create<K, T>(string clientName, string url);
    ...
    /// <param name="url">The URL for the API context.</param>
    /// <returns>An instance of <see cref="IApiContext{T}"/>.</returns>
    IApiContext<K, T> Create<K, T>(HttpClient http, string url);
    ...
    /// <typeparam name="T">The type of the entity.</typeparam>
    /// <returns>An instance of <see cref="IApiContext{T}"/>.</returns>
    IApiContext<K, T> CreateDefault<K, T>(string baseUrl);
